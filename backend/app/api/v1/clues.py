...
from app.schemas.clue import ClueCreate, ClueUpdate, ClueResponse
...
async def transfer_clue(clue_id: UUID, target: str, comment: Optional[str] = None, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    result = await db.execute(select(Clue).where(Clue.id == clue_id))
    clue = result.scalar_one_or_none()
    if not clue:
        raise HTTPException(status_code=404, detail="Clue not found")
    clue.status = "transferred"
    clue.transfer_target = target
    clue.transfer_date = func.now()
    clue.transfer_comment = comment
    await db.commit()
    await write_audit_log(db, current_user.id, "transfer", "clue", clue_id, {"target": target})
