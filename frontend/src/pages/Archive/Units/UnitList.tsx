...
interface UnitFormProps {
  unitTypeOptions: { label: string; value: string }[];
  unitLevelOptions: { label: string; value: string }[];
  parentUnitOptions: { label: string; value: string }[];
}

// Shared form row layout helper
const twoCol = (children: React.ReactNode) => (
  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>{children}</div>
);

// Shared form fields used by both Create and Edit modals
function UnitFormFields({ unitTypeOptions, unitLevelOptions, parentUnitOptions }: UnitFormProps) {
  return (
    <>
...
const UnitList: React.FC = () => {
  ...
  const handleCreateSubmit = async () => {
    try {
      ...
      await createUnit(collectComplexFields(values));
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const values = await form.validateFields();
      await updateUnit(editingUnit.id, collectComplexFields(values));
  ...
  const openEditModal = (record: Unit) => {
    ...
    if (record.tags && typeof record.tags === 'object') {
      Object.entries(record.tags).forEach(([k, v]) => { values[`tag_${k}`] = v; });
    }
    if (record.leadership && typeof record.leadership === 'object') {
      Object.entries(record.leadership).forEach(([k, v]) => { values[`leadership_${k}`] = v; });
    }
    if (record.contact && typeof record.contact === 'object') {
      Object.entries(record.contact).forEach(([k, v]) => { values[`contact_${k}`] = v; });
    }
